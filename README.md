...
2. This was my first project with typescript, although I couldn't use TS to it's absolute max due to lack of depth with Type System. I do belive my work should less prone to failing on production compared to just using plain old js. Most of the variables are type checked, and a lot of bugs were avoided during compile time. Although there seem to be a weird quirk between ESM and typescript, where you need to import modules with ".js" extension, although that only happened to me on production, was working ok locally.
